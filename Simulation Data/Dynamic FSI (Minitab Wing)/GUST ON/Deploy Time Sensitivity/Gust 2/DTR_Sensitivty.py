...
# Read the CSV files
data1 = pd.read_csv('0852_G2.csv')
data2 = pd.read_csv('0926_G2.csv')
data3 = pd.read_csv('1000_G2.csv')
data4 = pd.read_csv('1074_G2.csv')
data5 = pd.read_csv('G2_Clean.csv')
...
plt.figure(1)
plt.plot([x/100 - 1.29 for x in flow_time5], tip_displacement5, color='magenta')
plt.plot([x/100 - 1.29 for x in flow_time1], tip_displacement1, color='black')
plt.plot([x/100 - 1.29 for x in flow_time2], tip_displacement2, color='blue')
plt.plot([x/100 - 1.29 for x in flow_time3], tip_displacement3, color='red')
plt.plot([x/100 - 1.29 for x in flow_time4], tip_displacement4, color='green')
...
plt.legend(['Clean Wing ', 'DTR = -1.92 ', 'DTR = -0.46 ', 'DTR = 1.00 ','DTR = 2.46 '], fontsize = 16)
plt.xlabel('Flow Time (0.01s)', fontsize = 18)
plt.ylabel('Tip Displacement', fontsize = 18)
# plt.title('Tip Displacement Vs Time (Deployment Time Variation (s)) Gust G2')
plt.grid(True)
...
plt.figure(2)
plt.plot([x/100 - 1.29 for x in flow_time5], lift_coeff5, color='magenta')
plt.plot([x/100 - 1.29 for x in flow_time1], lift_coeff1, color='black')
plt.plot([x/100 - 1.29 for x in flow_time2], lift_coeff2, color='blue')
plt.plot([x/100 - 1.29 for x in flow_time3], lift_coeff3, color='red')
plt.plot([x/100 - 1.29 for x in flow_time4], lift_coeff4, color='green')


plt.axvline(x=0, color='black', linestyle='--', linewidth=1)  # Peak Cl
plt.text((12 + 1)/100, 0.5, f'Max Diff: {percentage_difference:.3f}%', color='black', fontsize = 16)
...
# plt.title('Lift Coeff. Vs Time (Deployment Time Variation (s)) Gust G2')
plt.grid()
...
lift_coeff_at_4 = [lift_coeff1[136], lift_coeff2[136], lift_coeff3[136], lift_coeff4[136]]
...
plt.xticks(configurations, ['-1.92', '-0.46', '1.00', '2.46'])
