...
# Read the CSV files
data1 = pd.read_csv('0852_G1.csv')
data2 = pd.read_csv('0926_G1.csv')
data3 = pd.read_csv('1000_G1.csv')
data4 = pd.read_csv('1074_G1.csv')
data5 = pd.read_csv('G1_Clean.csv')
...
plt.figure(1)
plt.plot([x/100 - 1.30  for x in flow_time5], tip_displacement5, color='magenta')
plt.plot([x/100 - 1.30  for x in flow_time1], tip_displacement1, color='black')
plt.plot([x/100 - 1.30  for x in flow_time2], tip_displacement2, color='blue')
plt.plot([x/100 - 1.30  for x in flow_time3], tip_displacement3, color='red')
plt.plot([x/100 - 1.30  for x in flow_time4], tip_displacement4, color='green')
...
plt.legend(['Clean Wing ', 'DTR = -4.00 ', 'DTR = -1.50 ', 'DTR = 1.00 ','DTR = 3.50 '], fontsize = 16)
plt.xlabel('Time (s)', fontsize = 18)
plt.ylabel('Tip Displacement', fontsize = 18)
# plt.title('Tip Displacement Vs Time (Deployment Time Variation (s)) Gust G1')
plt.grid(True)
...
plt.figure(2)
plt.plot([x/100 - 1.30 for x in flow_time5], lift_coeff5, color='magenta')
plt.plot([x/100 - 1.30  for x in flow_time1], lift_coeff1, color='black')
plt.plot([x/100 - 1.30  for x in flow_time2], lift_coeff2, color='blue')
plt.plot([x/100 - 1.30  for x in flow_time3], lift_coeff3, color='red')
plt.plot([x/100 - 1.30  for x in flow_time4], lift_coeff4, color='green')



plt.axvline(x=0, color='black', linestyle='--', linewidth=1)  # Peak Cl
plt.text((peak_cl_time + 1)/100, 0.49, f'Max Diff: {percentage_difference:.3f}%', color='black', fontsize = 14)
...
# plt.title('Lift Coeff. Vs Time (Deployment Time Variation (s)) Gust G1')
plt.grid()
...
lift_coeff_at_4 = [lift_coeff1[134], lift_coeff2[134], lift_coeff3[134], lift_coeff4[134]]
...
plt.xticks(configurations, ['-4.00', '-1.50', '1.00', '3.50'])
plt.xlabel('DTR', fontsize = 18)
plt.ylabel('Lift Coeff.', fontsize = 18)
# plt.title('Lift Coeff. at Gust(G1) Peak vs DTR Configurations')
plt.grid()
plt.show()